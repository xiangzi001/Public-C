...
template <typename T>
void SVD_EVD_QR_full(MAT<T>& A, MAT<T>& U, MAT<T>& E, MAT<T>& V, T eps) {
	if (A.rows > A.cols) {
		T* ur = new T[A.cols];
		//T(A)*A = V*S*T(V)
...
template <typename T>
void SVD_EVD_QR_thin(MAT<T>& A, MAT<T>& U, MAT<T>& E, MAT<T>& V, T eps) {
	if (A.rows > A.cols) {
		T* ur = new T[A.cols];
		//T(A)*A = V*S*T(V)
...
template <typename T>
void SVD_EVD_Jacobi_full(MAT<T>& A, MAT<T>& U, MAT<T>& E, MAT<T>& V, T eps) {
	if (A.rows > A.cols) {
		//T(A)*A = V*S*T(V)
...
template <typename T>
void SVD_EVD_Jacobi_thin(MAT<T>& A, MAT<T>& U, MAT<T>& E, MAT<T>& V, T eps) {
	if (A.rows > A.cols) {
		//T(A)*A = V*S*T(V)
		times_AT_B(A, A, E);

		EVD_Q_Jacobi(E, V, eps);
...
//THIN A(r, c) U(r, s) E(s, s) V(c, s)
//FULL  A(r, c) U(r, r) E(r, c) V(c, c)
template<typename T>
void SVD(MAT<T>& A, MAT<T>& U, MAT<T>& E, MAT<T>& V, T eps, SVD_type type) {
	...
	case _EVD_QR_full: {
		SVD_EVD_QR_full(A, U, E, V, eps);
		break;
	}
	case _EVD_QR_thin: {
		SVD_EVD_QR_thin(A, U, E, V, eps);
		break;
	}
	case _EVD_Jacobi_full: {
		SVD_EVD_Jacobi_full(A, U, E, V, eps);
		break;
	}
	case _EVD_Jacobi_thin: {
		SVD_EVD_Jacobi_thin(A, U, E, V, eps);
		break;
	}
	default:
		break;
